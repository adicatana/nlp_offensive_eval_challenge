...
def get_tokenized_corpus(corpus, f, is_stop=True):
    tokenized_corpus = []
...
def parse_dataset_task_a(filename):
    ...
    return train_corpus, train_labels_a
...
# Slang, keep alphanumeric and spaces only
...
def augment_untargeted(corpus, labels, n):
    ...
    for idx, label in enumerate(labels):
        if label == 0: 
            untargeted.append(corpus[idx])
       
    for i in range(n):
        r = random.randint(0, len(untargeted)) - 1      
        corpus.append(untargeted[r])
        labels.append(0)  
